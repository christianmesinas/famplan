...
def get_current_user():
    if 'user' in session:
        ...
        app.logger.debug(f"Checking user with email: {user_info['email']}")
        user = db.session.scalar(sa.select(User).where(User.email == user_info['email']))
        if not user:
            app.logger.debug("User not found in database, should have been created in callback")
        return user
    app.logger.debug("No user in session")
    return None

# Voeg deze contextprocessor toe
...
def create_user():
    ...
    if not user:
        ...
        db.session.add(user)
        db.session.commit()
        app.logger.debug(f"User created: {user.email}")  # Logging toegevoegd
...
# Overige routes aanpassen (verwijder @login_required en gebruik session['user'])
@app.route('/explore')
def explore():
    if 'user' not in session:
        return redirect(url_for('login'))
...
# Overige routes zoals /follow, /unfollow, /messages, etc. ook aanpassen
# Verwijder oude Flask-Login routes zoals /register, /reset_password_request
